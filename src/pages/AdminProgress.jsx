...
const ProgressTracking = () => {
  ...
  // Fetch user rating for stats
  const { data: userRatingData, isLoading: isRatingLoading, isError: isRatingError, error: ratingError } = useGetUserRatingQuery(user?.id);

  const departmentId = isSuperAdmin ? null : user?.department?._id || '60d0fe4f5311236168a109ca';

  // Fetch objectives
  const { data: objectives = [], isLoading, isError, error, refetch } = isSuperAdmin
    ? useGetObjectivesQuery()
    : useGetDepartmentObjectivesQuery({
        departmentId,
        ...filters,
  ...
  const calculateGoalCompletion = () => {
    ...
    objectives.forEach((objective) => {
      if (objective.subObjectives && Array.isArray(objective.subObjectives)) {
        console.log(`Objective ${objective._id} subObjectives:`, objective.subObjectives); // Debug subObjectives
  ...
  useEffect(() => {
    ...
    if (isRatingError) {
      ...
      toast.error(ratingError?.data?.error || 'Failed to load user rating');
    }
  }, [isError, error, isRatingError, ratingError, objectives, isLoading]);
  ...
  const handleApproveCompletion = async (objective) => {
    if (!isAdmin && !isSuperAdmin) {
      toast.error('Only admins or super admins can approve task completion');
      return;
    }
    if (!objective._id) {
  ...
  const handleRejectCompletion = async (objective) => {
    if (!isAdmin && !isSuperAdmin) {
      toast.error('Only admins or super admins can reject task completion');
      return;
    }
    if (!objective._id) {
      toast.error('Invalid objective ID');
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 rounded-lg">
      <div>
        ...
        {/* Stats Section for Super Admins */}
        ...
        <div className="space-y-6">
          {isLoading ? (
            ...
            <div className="text-center text-red-600">
              {isSuperAdmin ? 'Failed to load objectives' : 'You are not assigned to any department yet!'}
            </div>
              ...
              <div key={objective._id} className="bg-white rounded-lg shadow border border-gray-200 p-6">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center">
                    ...
                    >
                      {objective.title?.substring(0, 50) || 'Untitled'}...
                    </h2>
                    <span
                  ...
                  <div className="flex gap-3">
                    <p className="text-xs text-gray-600 mb-4">{objective.assignedTo?.name || 'Unassigned'}</p>
                    {isSuperAdmin && objective.department && (
                      <p className="text-xs text-gray-600 mb-4">Dept: {objective.department?.name || 'Unknown'}</p>
                    )}
                ...
                <p className="text-gray-600 mb-6 leading-relaxed">{objective.description || 'No description provided.'}</p>
                <div className="mb-4">
                  <div className="flex items-center justify-between text-sm text-gray-600 mb-1">
                    <span>{objective.progress || 0}%</span>
                    <span>
                      Last updated: {new Date(objective.updatedAt || objective.startDate || Date.now()).toLocaleDateString()}
                    </span>
                  </div>
                ...
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center text-gray-500">
                    ...
                    <span className="text-sm">
                      Due: {objective.endDate ? new Date(objective.endDate).toLocaleDateString() : 'Not set'}
                    </span>
                  </div>
        ...
        {/* Pagination */}
        {!isLoading && !isError && Array.isArray(objectives) && objectives.length > 0 && (
          <div className="mt-6 flex justify-center space-x-2">
            ...
            <span className="px-4 py-2 text-sm">
              Page {filters.page} of {Math.ceil(objectives.length / filters.limit)}
            </span>
            <button
              onClick={() => handlePageChange(filters.page + 1)}
              disabled={filters.page === Math.ceil(objectives.length / filters.limit)}
              className="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Next
            </button>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-5xl">
              ...
              <div className="space-y-4 grid grid-cols-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">{selectedObjective.title || 'Untitled'}</p>
                </div>
                <div>
                ...
                {isSuperAdmin && selectedObjective.department && (
                  <div>
                    <h4 className="text-sm font-medium text-gray-700">Department</h4>
                    <p className="text-sm text-gray-600">{selectedObjective.department?.name || 'Unknown'}</p>
                  </div>
                )}
                <div>
                  <h4 className="text-sm font-medium text-gray-700">{isAdmin || isSuperAdmin ? 'Created By' : 'Manager'}</h4>
                  <p className="text-sm text-gray-600">
                    {isAdmin || isSuperAdmin
                      ? `${selectedObjective.createdBy?.name || 'Unknown'} (${selectedObjective.createdBy?.email || 'N/A'})`
                      : user?.manager
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {selectedObjective.assignedTo?.name || 'Unassigned'} ({selectedObjective.assignedTo?.email || 'N/A'})
                  </p>
                </div>
                <div>
              ...
              <div className="flex justify-end mt-6 space-x-3">
                {(isAdmin || isSuperAdmin) && selectedObjective.progress >= 75 && (
                  <>
                    <button
