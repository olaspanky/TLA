...
const UserList = ({ setTeam, team }) => {
  ...
  const handleChange = (el) => {
    setSelectedUsers(el);
    setTeam(el?.map((u) => u._id)); // setting selected team ids
  };

  useEffect(() => {
    if (data && !isLoading) {
      // Automatically select the current user if not already in the team
      const isCurrentUserSelected = selectedUsers.some(u => u._id === user._id);
      
      if (!isCurrentUserSelected) {
        // If no team members are selected, default to selecting the current user
        const initialSelected = team?.length > 0 ? team : [user];
        setSelectedUsers(initialSelected);
        setTeam(initialSelected.map(u => u._id));
      }
    }
  }, [data, isLoading, team, user]);
